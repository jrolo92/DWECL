var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
var ServicioProductos = /** @class */ (function () {
    function ServicioProductos() {
        this.url = "http://localhost:3000/productos";
    }
    ServicioProductos.prototype.funAjax = function () {
        return __awaiter(this, void 0, void 0, function () {
            var response, datos, e_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        return [4 /*yield*/, fetch(this.url)];
                    case 1:
                        response = _a.sent();
                        return [4 /*yield*/, response.json()];
                    case 2:
                        datos = _a.sent();
                        return [2 /*return*/, datos];
                    case 3:
                        e_1 = _a.sent();
                        // Si hay algun error lo muestra por consola
                        console.log("Error: ".concat(e_1));
                        // Devolvemos array vacío si hay un error
                        return [2 /*return*/, []];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    return ServicioProductos;
}());
var MostrarProductos = /** @class */ (function () {
    function MostrarProductos() {
    }
    MostrarProductos.prototype.pintarTabla = function (productos) {
        var tbody = document.getElementById('cuerpoTabla');
        if (tbody) {
            tbody.innerHTML = "";
            productos.forEach(function (p) {
                var fila = "<tr>\n                                <td>".concat(p.id, "</td>\n                                <td>").concat(p.nombre, "</td>\n                                <td>").concat(p.precio, "</td>\n                                <td>").concat(p.stock, "</td>\n                             </tr>");
                tbody.innerHTML += fila;
            });
        }
    };
    return MostrarProductos;
}());
// Iniciamos las clases
var servicio = new ServicioProductos();
var vista = new MostrarProductos();
// Evento para el botón
var boton = document.getElementById("boton");
// En vez de comprobar que boton tiene contenido se le puede poner boton?.addEventListener
if (boton) {
    boton.addEventListener("click", function () { return __awaiter(_this, void 0, void 0, function () {
        var datos, e_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, servicio.funAjax()];
                case 1:
                    datos = _a.sent();
                    vista.pintarTabla(datos);
                    return [3 /*break*/, 3];
                case 2:
                    e_2 = _a.sent();
                    console.error("Error al cargar: ", e_2);
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    }); });
}
