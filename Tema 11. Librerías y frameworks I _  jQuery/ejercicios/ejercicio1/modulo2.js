...
export function cargarNombres() {
    ...
    funAjax("jsonGET.php", function fun1 (obj){
        ...
        let opcion = $("#sel");
        opcion.append("<option>Seleccione nombre</option>");
        // Recorremos el JSON y pintamos cada nombre en el select
        $.each(obj, function(i, opciones){
            opcion.append(`<option value=${opciones.id}>${opciones.nombre}</option>`)
        });
